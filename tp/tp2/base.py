...
# ------------------------------------------------------------------
#   Conservamos el mismo flujo y comentarios del archivo base
#   (modelo XGBoost + validación con AUC + anti-fuga)
# ------------------------------------------------------------------
...
# ------------------------------------------------------------------
#   Ingeniería de características (rica, sin fuga)
# ------------------------------------------------------------------

def build_features(df):
    '''
    Features enriquecidas sin fuga de información y con backoff.
    '''
    ...
    #   Codificación cíclica (mejor que hour/dow crudos)
    ...
    #   Target + indicador de test
    df["target"] = (df["reason_end"] == "fwdbtn").astype("Int8")
    df["is_test"] = df["reason_end"].isna()

    #   === Tasas históricas por usuario (sin futuro) con prior fuerte ===
    ...
    df["user_forward_rate_hist"] = (w_user * user_rate_raw + (1 - w_user) * global_rate).fillna(global_rate).astype("float32")

    #   === Rolling recientes del usuario (sin fuga) ===
    ...
    #   Tiempo desde el último forward del usuario
    ...
    df["secs_since_last_forward"] = (df["ts_unix"] - last_fwd_cum).clip(lower=0).fillna(df["secs_since_prev"])\
                                        .astype("float32")
    df["log1p_secs_since_last_forward"] = np.log1p(df["secs_since_last_forward"]).astype("float32")

    #   === Afinidad usuario–artista (expanding mean con backoff) ===
    ...
    #   === Backoff globales SOLO con train (anti-fuga) ===
    train_mask = ~df["is_test"]

    # Frecuencia de artista (popularidad)
    if "master_metadata_album_artist_name" in df.columns:
        freq_artist = df.loc[train_mask, "master_metadata_album_artist_name"].value_counts()
    ...
    if "master_metadata_album_artist_name" in df.columns:
        art_grp = (
            df.loc[train_mask]
              .groupby("master_metadata_album_artist_name")["target"]
              .agg(["mean","count"]).rename(columns={"mean":"art_mean","count":"art_cnt"})
        )
        df = df.merge(art_grp, how="left",
                      left_on="master_metadata_album_artist_name", right_index=True)
    else:
        df["art_mean"], df["art_cnt"] = global_rate, 0

    # country → mean,count
    if "conn_country" in df.columns:
        ctry_grp = (
            df.loc[train_mask]
              .groupby("conn_country")["target"]
              .agg(["mean","count"]).rename(columns={"mean":"ctry_mean","count":"ctry_cnt"})
        )
        df = df.merge(ctry_grp, how="left", left_on="conn_country", right_index=True)
    else:
        df["ctry_mean"], df["ctry_cnt"] = global_rate, 0

    # artist × country con backoff → artist/country/global
    if all(col in df.columns for col in ["master_metadata_album_artist_name", "conn_country"]):
        ac_grp = (
            df.loc[train_mask]
              .groupby(["master_metadata_album_artist_name", "conn_country"])['target']
              .agg(["mean","count"]).rename(columns={"mean":"ac_mean","count":"ac_cnt"})
        )
        df = df.merge(
            ac_grp,
            how="left",
            left_on=["master_metadata_album_artist_name","conn_country"],
            right_index=True,
        )
    else:
        df["ac_mean"], df["ac_cnt"] = np.nan, 0

    base_bo = 0.7 * df["art_mean"].fillna(global_rate) + 0.3 * df["ctry_mean"].fillna(global_rate)
    w_ac = df["ac_cnt"] / (df["ac_cnt"] + 50.0)
    df["artist_country_rate_bo"] = (w_ac * df["ac_mean"].fillna(base_bo) + (1 - w_ac) * base_bo).astype("float32")

    # platform backoff → global
    if "platform" in df.columns:
        plat_grp = (
            df.loc[train_mask]
              .groupby("platform")["target"]
              .agg(["mean","count"]).rename(columns={"mean":"plat_mean","count":"plat_cnt"})
        )
        df = df.merge(plat_grp, how="left", left_on="platform", right_index=True)
        w_plat = df["plat_cnt"] / (df["plat_cnt"] + 100.0)
        df["platform_rate_bo"] = (w_plat * df["plat_mean"].fillna(global_rate) + (1 - w_plat) * global_rate).astype("float32")
    else:
        df["platform_rate_bo"] = global_rate

    #   Sesiones por usuario (corte a 30 min) – señales causales
    ...
    num_cols = [
        ...
        "artist_freq","artist_country_rate_bo","platform_rate_bo",
...
# ------------------------------------------------------------------
#   Entrenar y predecir (pre fuera del pipeline por compatibilidad)
# ------------------------------------------------------------------

def train_and_predict(train_df, test_df, cat_cols, num_cols):
    '''
    Preprocesamiento manual + XGBoost; imprime AUC en validación temporal.
    '''
...
def main():
    print("=== Arrancando pipeline (XGBoost mejorado) ===")

    df = load_competition_datasets(
        COMPETITION_PATH,
        sample_frac=1.0,   # usar todo el train para el submit
        random_state=1234
    )
